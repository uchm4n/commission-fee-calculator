# Homework assignment
## Situation
P*** allows private and business clients to `deposit` and `withdraw` funds to and from P*** accounts in multiple currencies. Clients may be charged a commission fee.
...
1. Third party frameworks, libraries, dependencies, tools are **not required** but **allowed**.
    ...
    2. If your application is not based on framework skeleton, your project must use this skeleton as a bootstrap.
...
8. Do not use P*** name in titles, descriptions or the code itself. This helps others to find the libraries that are really related to our services and/or are developed and maintained by our team.

# Evaluation Criteria
1. Requirements considered as met by P*** development team. It would be best if all requirements are met;
...
You can upload the source (i.e. to GitHub) publicly, but **please refrain from using P*** name** in title, description or source code.

Submit the task in your chosen format (link to versioned code in GitHub/Google Drive or code in zip file, etc.) by sending it to the email provided by P*** HR Coordinator or your Agency. Please also include approx. how much time was spent to complete the task.